...
struct PageStats
{
  ...
  PageStats(uint8_t n_pfx, bool nulls, dict_index_t *index) :
  ...
  uint32_t scan(const dict_index_t *index, const page_t *page) noexcept;
...
@tparam		leaf			Whether leaf page has been
					passed
...
@return child page number or 0 if the page is empty and doesn't contain
user records  */
template<bool leaf>
uint32_t PageStats::scan(const dict_index_t *index, const page_t *page)noexcept
{
...
{
	dict_index_t*	index = btr_cur_get_index(cur);
	...
	for (;;) {
		...
		uint32_t child_page_no = scan<false>(index, page);
	...
	n_diff_data_t*		n_diff_data)const noexcept
{
	...
	for (i = 0; i < n_diff_data->n_leaf_pages_to_analyze; i++) {
		...
		PageStats *leaf_stats = new PageStats((uint8_t)n_prefix,
						      nulls_distinct(), index);
